...
// Struct to hold the RGB colour data the program calculates, values should be between 0 and 255 inclusive
type RGB struct {
	R float64
	G float64
	B float64
...
// Calculates the difference between two RGB colours by treating them as vectors in 3D space
func distBetween(c1, c2 *RGB) float64 {
...
func distBetweenRGBA(c1, c2 RGBA) float64 {
	...
	return math.Sqrt(r_diff + g_diff + b_diff + a_diff)
}

// Reads JPEG and PNG images into a set of RGB vectors
func ImgToRGBPixels(path string) *gs.Set {
...
func RecreateImageFromColours(imgpath string, colours []*RGB) {
...
// Draws a rectangle of 200x200 squares of the colours input into the function
func DrawRectangleColours(colours []*RGB, path string) {
...
// Draws a rectangle of 200x200 squares of the colours input into the function
func DrawRectangleGreyscale(colours []uint8, path string) {
	numColours := len(colours)
	img := image.NewGray(image.Rect(0, 0, 200*numColours, 200))

	for i := 0; i < len(colours); i++ {
		c := image.NewUniform(color.Gray{colours[i]})
		draw.Draw(img, image.Rect(200*i, 0, 200*i+200, 200), c, image.ZP, draw.Src)
	}

	// TODO validate the path, make function to save the image
	toimg, err := os.Create(path)
	if err != nil {
		fmt.Printf("Error: %v", err)
		return
